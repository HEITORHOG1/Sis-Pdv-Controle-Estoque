...
namespace Sis_Pdv_Controle_Estoque_Form.Utils
{
    public static class ProdutoLogger
    {
        ...
        {
            var message = $"Erro de valida��o - Campo: {field}, Erro: {error}";
        ...
        {
            var message = $"Atualiza��o Estoque - Produto: {produtoNome}, Opera��o: {operacao}, " +
                         $"Anterior: {quantidadeAnterior}, Nova: {quantidadeNova}, Diferen�a: {quantidadeNova - quantidadeAnterior}";
        ...
        {
            var message = $"Atualiza��o Pre�o - Produto: {produtoNome}, Tipo: {tipo}, " +
                         $"Anterior: R$ {precoAnterior:F2}, Novo: R$ {precoNovo:F2}, " +
                         $"Diferen�a: R$ {precoNovo - precoAnterior:F2}";
        ...
        {
            var message = $"Valida��o C�digo Barras - C�digo: {codigoBarras}, V�lido: {isValido}";
            if (!string.IsNullOrEmpty(produtoNome))
        ...
        public static void LogMargemLucro(string produtoNome, decimal PrecoCusto, decimal PrecoVenda, decimal margem)
        {
            ...
            var message = $"An�lise Margem - Produto: {produtoNome}, " +
        ...
        {
            var message = $"Altera��o Categoria - Produto: {produtoNome}, " +
        ...
        {
            var message = $"Altera��o Fornecedor - Produto: {produtoNome}, " +
        ...
        {
            var message = $"Busca por C�digo - C�digo: {codigoBarras}, Encontrado: {encontrado}";
            if (encontrado)
        ...
        {
            var message = $"Performance - Opera��o: {operacao}, Dura��o: {duracao.TotalMilliseconds}ms";
            if (quantidadeItens > 0)
                message += $", Itens: {quantidadeItens}, M�dia: {duracao.TotalMilliseconds / quantidadeItens:F2}ms/item";
        ...
        private static void WriteLog(string level, string message, string operation)
        {
            try
            {
                ...
                // Limpa logs antigos (mant�m apenas os �ltimos 30 dias)
                CleanOldLogs();
            }
            catch (Exception ex)
            {
        ...
        {
            var message = $"Gera��o Relat�rio - Tipo: {tipoRelatorio}, Itens: {quantidadeItens}";
        ...
        {
            var message = $"Importa��o - Arquivo: {Path.GetFileName(arquivo)}, " +
        ...
        {
            var message = $"Exporta��o - Arquivo: {Path.GetFileName(arquivo)}, " +
